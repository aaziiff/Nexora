...
import { ShieldCheck, Truck, QrCode, Copy, Check, ArrowRight, ArrowLeft } from 'lucide-react';
      ...
      <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="bg-ivory-100 p-6 sm:p-8 rounded-xl border border-stone/30 shadow-sm space-y-5">
            ...
            {/* UPI Instruction Box */}
            {paymentMethod === 'UPI' && (
              <div className="p-5 rounded-xl bg-ivory-200/80 border border-stone/40 space-y-4 text-xs animate-fade-in">
                <div className="flex items-center gap-2 text-charcoal-800 font-medium">
                  <QrCode className="w-4 h-4 text-sage-700" />
                  <span>NEXORA Verified UPI Account</span>
                </div>

                <div className="p-3 bg-ivory-50 rounded border border-stone/40 flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-charcoal-500 uppercase tracking-widest block">UPI ID</span>
                    <span className="text-xs font-mono font-semibold text-charcoal-950">{settings.upi_id}</span>
                  </div>
                  <button
                    type="button"
                    onClick={handleCopyUpi}
                    className="inline-flex items-center gap-1 text-[11px] uppercase tracking-wider font-semibold text-charcoal-800 bg-stone/30 hover:bg-stone/50 px-3 py-1.5 rounded transition-colors"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 12-digit UTR reference from GPay/PhonePe"
                    className="w-full px-3.5 py-2.5 bg-ivory-50 border border-stone/60 rounded text-xs text-charcoal-900 font-mono focus:outline-none focus:border-charcoal-900"
                  />
                  <p className="text-[10px] text-charcoal-500 mt-1">
                    Your payment reference is verified by our concierge team before dispatch.
                  </p>
                </div>
              </div>
            )}
